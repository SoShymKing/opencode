...
### Σχετικά με το branch `ws`

Αυτό το README τεκμηριώνει το branch `ws` ως branch τύπου διανομής όσο οι αλλαγές του παραμένουν εκτός του upstream `dev`.

Κύρια σημεία σε σύγκριση με το `dev`:

- Χειρισμός session χωρίς απόκριση και abort retry.
- Ανάκτηση μετά από restart του TUI interrupt worker και σταθερότητα spinner/rendering.
- Επαναφορά αρχειοθετημένων sessions σε ροές TUI, Web, API και SDK.
- Ρυθμιζόμενο χρονικό εύρος για τη λίστα sessions του TUI.
- Τοπικός καθαρισμός smoke/config στο `.opencode`.

Οι παρακάτω εντολές εγκατάστασης δεν αλλάζουν· χρησιμοποιήστε τον πηγαίο κώδικα του branch `ws` αν θέλετε αυτές τις branch-specific αλλαγές.
