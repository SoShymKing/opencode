...
Цей README описує гілку `ws` як гілку в стилі дистрибутива, доки її зміни залишаються поза upstream `dev`.

Основне порівняно з `dev`:

- Обробка відсутності відповіді сесії та повторів після abort.
- Відновлення після перезапуску TUI interrupt worker і стабільність spinner/rendering.
- Відновлення архівованих сесій у потоках TUI, Web, API та SDK.
- Налаштовуваний часовий діапазон для списку сесій TUI.
- Локальне очищення smoke/config у `.opencode`.

Команди встановлення нижче не змінено; використовуйте джерельний код гілки `ws`, якщо потрібні ці специфічні для гілки зміни.
