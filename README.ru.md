...
Этот README описывает ветку `ws` как ветку в стиле дистрибутива, пока ее изменения остаются вне upstream `dev`.

Основное по сравнению с `dev`:

- Обработка отсутствия ответа сессии и повторов после abort.
- Восстановление после перезапуска TUI interrupt worker и стабильность spinner/rendering.
- Восстановление архивных сессий в потоках TUI, Web, API и SDK.
- Настраиваемый временной диапазон для списка сессий TUI.
- Локальная очистка smoke/config в `.opencode`.

Команды установки ниже не изменены; используйте исходники ветки `ws`, если нужны эти изменения, специфичные для ветки.
