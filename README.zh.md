...
在这些改动尚未进入上游 `dev` 期间，本 README 将 `ws` 分支作为类似发行版的分支来说明。

相较于 `dev` 的主要内容：

- 会话无响应与 abort 重试处理。
- TUI interrupt worker 重启恢复，以及 spinner/渲染稳定性改进。
- TUI、Web、API、SDK 流程中的归档会话恢复。
- 可配置的 TUI 会话列表查询时间范围。
- 本地 `.opencode` smoke/config 清理。

下方安装命令保持不变；如果需要这些分支专属改动，请使用 `ws` 分支源码。
