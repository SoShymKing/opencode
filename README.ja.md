...
この README は、変更が upstream `dev` にまだ入っていない間、`ws` ブランチを配布版のようなブランチとして説明します。

`dev` と比べた主な内容:

- セッション無応答と abort の再試行処理。
- TUI interrupt worker の再起動復旧と spinner/レンダリング安定化。
- TUI、Web、API、SDK フローでのアーカイブ済みセッション復元。
- TUI セッション一覧の取得期間設定。
- ローカル `.opencode` smoke/config の整理。

下のインストールコマンドは変更していません。これらのブランチ固有の変更が必要な場合は、`ws` ブランチのソースを使用してください。
