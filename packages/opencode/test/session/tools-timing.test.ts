...
import { model } from "./prompt-history-conversion.fixtures"

const callID = "call-test"
const timingSessionID = SessionID.make("ses_test")
const timingMessageID = MessageID.ascending()
const partID = PartID.ascending()

const timingAgent: Agent.Info = {
  name: "build",
  mode: "primary",
  options: {},
...
const timingSession: Session.Info = {
  id: timingSessionID,
  slug: "tools-timing-test",
  projectID: ProjectV2.ID.make("tools-timing-test"),
  directory: "/tmp",
  title: "tools timing test",
  version: "test",
...
const timingLayer = Layer.mergeAll(
...
timingIt.effect("preserves running tool start time across metadata updates", () =>
  Effect.gen(function* () {
    const state: SessionV1.ToolPart = {
      id: partID,
      sessionID: timingSessionID,
      messageID: timingMessageID,
      type: "tool",
      tool: "timing",
      callID,
      state: {
    ...
    const processor = {
      message: {
        ...
        modelID: timingModel.id,
        providerID: timingModel.providerID,
        time: { created: 1 },
