...
يوثق هذا README فرع `ws` كفرع شبيه بالتوزيع ما دامت تغييراته خارج upstream `dev`.

ابرز النقاط مقارنة مع `dev`:

- التعامل مع الجلسات التي لا تستجيب واعادة المحاولة بعد abort.
- الاسترداد بعد اعادة تشغيل TUI interrupt worker وتحسين ثبات spinner/rendering.
- استعادة الجلسات المؤرشفة عبر مسارات TUI و Web و API و SDK.
- نطاق زمني قابل للضبط لقائمة جلسات TUI.
- تنظيف ملفات smoke/config المحلية داخل `.opencode`.

اوامر التثبيت ادناه لم تتغير؛ استخدم مصدر فرع `ws` اذا كنت تريد هذه التغييرات الخاصة بالفرع.
