...
README นี้อธิบายสาขา `ws` เป็นสาขาแบบ distribution ในช่วงที่การเปลี่ยนแปลงยังไม่ได้เข้า upstream `dev`

จุดเด่นเมื่อเทียบกับ `dev`:

- การจัดการ session ที่ไม่ตอบสนองและการ retry หลัง abort
- การกู้คืนหลัง TUI interrupt worker restart และความเสถียรของ spinner/rendering
- การกู้คืน session ที่เก็บถาวรใน flow ของ TUI, Web, API และ SDK
- ช่วงเวลาที่กำหนดค่าได้สำหรับรายการ session ของ TUI
- การล้าง smoke/config ภายใน `.opencode` ในเครื่อง

คำสั่งติดตั้งด้านล่างไม่ได้เปลี่ยนแปลง หากต้องการการเปลี่ยนแปลงเฉพาะสาขานี้ ให้ใช้ซอร์สจากสาขา `ws`
